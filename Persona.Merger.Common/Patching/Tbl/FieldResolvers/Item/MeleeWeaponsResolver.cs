...
namespace Persona.Merger.Patching.Tbl.FieldResolvers.Item;

public struct MeleeWeaponsResolver : IEncoderFieldResolver
{
    public bool Resolve(nuint offset, out int moveBy, out int length)
    {
        ...
        if (offsetInStruct is >= 20 and < 26 or >= 44 and < 46)
        {
            // u8s
            moveBy = 0;
            length = 1;
